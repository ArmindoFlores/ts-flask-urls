...
def map_urls(out_dir: str, endpoint: str):
    ...
    with open(os.path.join(out_dir, "types.ts"), "w") as types_f:
        with open(os.path.join(out_dir, "apis.ts"), "w") as api_f:
            code_writer = CodeWriter(types_f, api_f, endpoint)
            result = code_writer.write((FlaskAnnotationsParser(current_app, rule) for rule in rules))
            if not result:
